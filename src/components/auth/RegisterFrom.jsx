...
const RegisterForm = () => {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    const submitFormData = { ...exactFormData, role };
    console.log("Submitting registration form:", submitFormData);
  };

  return (
    <div className="w-full">
      <Form className="flex w-full flex-col gap-5" onSubmit={handleRegisterSubmit}>
        <TextField
          isRequired
          name="name"
          type="text"
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="grid grid-cols-2 gap-3.5 w-full">
            ...
            >
              <div 
                className={`p-2.5 rounded-lg shrink-0 transition-colors duration-300 ${
                  role === 'seeker' ? 'bg-[#00a6fb]/15 text-primary' : 'bg-zinc-800/85 text-zinc-400'
                }`}
              >
                <FiSearch className="text-base" />
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-xs font-semibold text-white tracking-wide">Seeker</span>
                <span className="text-[10px] text-zinc-500 font-light truncate">Looking for roles</span>
              </div>
            </div>
            ...
            >
              <div 
                className={`p-2.5 rounded-lg shrink-0 transition-colors duration-300 ${
                  role === 'recruiter' ? 'bg-[#00a6fb]/15 text-primary' : 'bg-zinc-800/85 text-zinc-400'
                }`}
              >
                <FiBriefcase className="text-base" />
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-xs font-semibold text-white tracking-wide">Recruiter</span>
                <span className="text-[10px] text-zinc-500 font-light truncate">Hiring top talent</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <p className="text-center text-xs text-zinc-500 mt-6 font-light">
        ...
        <Link href="/login" className="text-primary hover:text-[#009ae8] hover:underline font-semibold ml-1 transition-all">
          Sign In
        </Link>
      </p>
    </div>
